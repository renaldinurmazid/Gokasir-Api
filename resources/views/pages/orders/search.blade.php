...
@section('title', 'GoKasir - Cari Menu')
...
@section('content')
    ...
    <header class="h-[52px] bg-white border-b border-slate-100 shadow-sm sticky top-0 z-50 px-4 flex items-center gap-3">
        ...
        <h1 class="font-bold text-sm text-slate-800 tracking-wide uppercase">Cari Menu</h1>
    </header>

    <!-- Scrollable content wrapper -->
    <div class="space-y-4 flex-1 pb-24">
        
        <!-- Search Input Field Container -->
        <div class="px-4 pt-4">
            <form action="/order/{{ $tableCode }}/search" method="GET" class="relative">
                <input type="text" name="q" value="{{ $searchQuery }}" placeholder="Cari makanan, minuman, cemilan..." autofocus
                ...
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-slate-400 absolute left-4 top-3.5 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
        ...
        <main class="px-4 space-y-4">
            @if ($searchQuery !== '')
                <div class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                    Hasil pencarian untuk: <span class="text-brand-500 font-extrabold normal-case">"{{ $searchQuery }}"</span>
                </div>
                ...
                @else
                    <!-- Query Empty State (No Results found) -->
                    <div class="text-center py-16 space-y-4">
                        <div class="w-20 h-20 bg-slate-100 rounded-full flex items-center justify-center mx-auto text-slate-400">
                            <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-search-x"><circle cx="11" cy="11" r="8"/><path d="m16 16 5.6 5.6"/><path d="m3.4 18.6 15.2-15.2"/></svg>
                        </div>
                        <div class="space-y-1">
                            <h3 class="font-bold text-slate-700 text-sm">Menu Tidak Ditemukan</h3>
                            <p class="text-xs text-slate-400 max-w-[200px] mx-auto leading-normal">
                                Coba cari dengan kata kunci lain seperti ayam, nasi, kopi, es, atau jus.
                            </p>
                        </div>
                    </div>
                @endif
            @else
                ...
                <div class="text-center py-20 space-y-4">
                    <div class="w-20 h-20 bg-slate-100 rounded-full flex items-center justify-center mx-auto text-slate-400 animate-pulse">
                        <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-search"><circle cx="11" cy="11" r="8"/><line x1="21" x2="16.65" y1="21" y2="16.65"/></svg>
                    </div>
                    <div class="space-y-1">
                        <h3 class="font-bold text-slate-700 text-sm">Cari Menu Favorit</h3>
                        <p class="text-xs text-slate-400 max-w-[220px] mx-auto leading-normal">
                            Masukkan nama hidangan makanan atau minuman lezat untuk menemukannya dengan cepat.
                        </p>
                    </div>
                </div>
            @endif
    ...
    <nav class="h-[64px] bg-white border-t border-slate-100 shadow-md fixed bottom-0 inset-x-0 max-w-[480px] mx-auto z-50 flex justify-around items-center">
        <!-- Tab 1: Menu -->
        <a href="/order/{{ $tableCode }}" class="flex-1 h-full flex flex-col items-center justify-center text-slate-400 hover:text-slate-500">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
            </svg>
            <span class="text-[10px] font-semibold mt-1 tracking-wide">Menu</span>
        </a>
